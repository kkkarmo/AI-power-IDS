        ...
        try:
            ...
            with open(PUBLIC_IPS_FILE, 'w') as f:
                ...
                for domain in domains:
                    f.write(f"{domain}\n")
            logging.debug(f"Public IPs and domains extracted and written to {PUBLIC_IPS_FILE}")
...
class IPFileHandler(FileSystemEventHandler):
    ...
    def process_ip_file(self, file_path):
        ...
        try:
            with open(file_path, 'r') as f:
                ips_domains = f.read().splitlines()
            logging.debug(f"IPs and domains found: {ips_domains}")
            
            for item in ips_domains:
                if self.should_check_ip(item):
                    result = self.check_ip_or_domain(item)
                    self.write_result(result)
                    logging.debug(result)  # Log result
                    self.checked_ips[item] = datetime.now()
                    self.save_checked_ips()
                    sleep(15)  # Wait 15 seconds between checks
                else:
                    logging.debug(f"Skipping item (recently checked): {item}")
        except Exception as e:
            logging.error(f"Error processing file: {str(e)}")
            traceback.print_exc()

    def should_check_ip(self, item):
        last_checked = self.checked_ips.get(item)
    ...
    def check_ip_or_domain(self, item):
        ...
        try:
            if ipaddress.ip_address(item):
                url = f"https://api.metadefender.com/v4/ip/{item}"
            else:
                url = f"https://api.metadefender.com/v4/domain/{item}"
            
            response = requests.get(url, headers=headers)
            logging.debug(f"Response status code: {response.status_code}")
            if response.status_code == 200:
                data = response.json()
                return f"Item: {item}, Reputation: {data.get('reputation', 'Unknown')}"
            elif response.status_code == 429:
                logging.debug(f"Rate limit exceeded for item: {item}. Waiting before retry...")
                sleep(60)  # Wait for 60 seconds before retrying
                return self.check_ip_or_domain(item)  # Retry the request
            else:
                return f"Item: {item}, Error: Unable to fetch results, status code: {response.status_code}"
        except Exception as e:
            logging.error(f"Error checking item {item}: {str(e)}")
            traceback.print_exc()
            return f"Item: {item}, Error: {str(e)}"
...
def save_tavily_results_to_database(data):
    ...
    for ip, result in data.items():
        cursor.execute(sql.SQL("""
            INSERT INTO results (ip, result) VALUES (%s, %s)
...
def scheduled_task():
    process_ips()
    # Add MetaDefender checking
    ip_event_handler = IPFileHandler(METADEFENDER_API_KEY)
    ip_event_handler.process_ip_file(PUBLIC_IPS_FILE)
